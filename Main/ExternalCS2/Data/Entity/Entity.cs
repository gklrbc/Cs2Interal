...
public class Entity : EntityBase
{
    ...
    {
        // Если твой дампер/парсер вытягивает имя класса сущности (DesignerName)
        if (string.IsNullOrEmpty(this.CurrentWeaponName)) return false;

        string name = this.CurrentWeaponName.ToLower();
        return name.Contains("projectile") || name.Contains("grenade") || name.Contains("molotov");
    }

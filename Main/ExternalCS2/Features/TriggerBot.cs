...
public sealed class TriggerBot : ThreadedServiceBase
{
    ...
    protected override void FrameAction()
    {
        ...
        // Передаем актуальный конфиг в метод выстрела
        ExecuteTrigger(config);
    ...
    private void ExecuteTrigger(ConfigManager config)
    {
        // Качественный легитный авто-стоп через проверку скорости вектора движения
        if (config.TriggerAutoStop && _gameData.Player != null)
        {
            var velocity = _gameData.Player.Velocity;
            // Рассчитываем горизонтальную скорость персонажа (оси X и Y)
            double currentSpeed = Math.Sqrt(velocity.X * velocity.X + velocity.Y * velocity.Y);

            // Если скорость выше 15, разброс слишком велик — пропускаем выстрел, 
            // пока игрок сам не затормозит (контр-стрейфом или отпустив клавиши)
        ...
        // Задержка перед выстрелом (в кадрах)
